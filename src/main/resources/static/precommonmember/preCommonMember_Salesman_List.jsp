...
//修改页面跳转
function updateSlesman(id){
	var  action = "/admin/preCommonMember/salesmaniew/"+id;    // 通过id查询需要修改的bean信息
...
//修改后保存
function update(){
	var jsonData = $("#salesman_updateForm").serialize();
    var params = decodeURIComponent(jsonData,true);//处理时间10:00,提交的时候乱码 103A00
	var bv = $("#salesman_updateForm").data("bootstrapValidator");
	//手动触发验证
	bv.validate();
	var action ="${pageContext.request.contextPath}/admin/preCommonMember/salesmanupdate";
	if(bv.isValid()){
		$.post(action, params, function(data){
		  if(data.meta.code=="88"){
       		  alert(data.meta.message);
	       	  window.location.href="${pageContext.request.contextPath}/admin/preCommonMember/salesman_list";
...
$(function(){
	...
	$("#issalesman_select").val("${preCommonMember.issalesman}");
...
<body style="font-size: 12px; font-family: 微软雅黑">
	...
	<div class="container" style="width: 80%;">
		<div style="float: left;">
			<form method="post" action="salesman_list" id="form-select" class="form-inline">
				<div style="margin-top: 18px;margin-bottom: 10px">
                    ...
                    <div class="form-group">
                        <label>是否业务员:</label>
                        <select class="form-control" name="issalesman" id="issalesman_select">
                            <option value="">全部</option>
                            <option value="1" <c:if test="${preCommonMember.issalesman==1}">selected</c:if>>是</option>
                            <option value="0" <c:if test="${preCommonMember.issalesman==0}">selected</c:if>>否</option>
                        </select>&nbsp;&nbsp;
                    </div>
					...
					<tbody>
						...
						<c:forEach items="${pagehelper.list}" var="precommonmember"
							...
							<%-- 	<td>${precommonmember.realname}</td> --%>
								...
								<td>
									<c:if test="${precommonmember.issalesman  eq '1'}"><span style="color: blue">是</span></c:if>
									<c:if test="${precommonmember.issalesman eq '0'}"><span style="color: red">否</span></c:if>
								</td>

								 <td class="remark-p" title="${precommonmember.salesmanremark }">${precommonmember.salesmanremark }</td>
								 <td>
									<button class="btn" data-backdrop="static"
										data-toggle="modal" data-dismiss="modal"
										data-target="#updateModal"
										onclick="updateSlesman('${precommonmember.uid}')">设置</button>
								</td>
							</tr>
