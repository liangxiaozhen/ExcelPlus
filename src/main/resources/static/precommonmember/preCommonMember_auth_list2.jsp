...
function deatilUI(id){
	var  action =  "/admin/preCommonMember/auth_detail2/"+id;
...
//权限修改页面跳转
function upateUI(id){
	var  action =  "/admin/preCommonMember/userPermissionsupdateView2/"+id;
	$.post(action, function(data){
...
//修改后保存
function update(){
	var jsonData = $("#auth_updateForm2").serialize();
    var params = decodeURIComponent(jsonData,true);//处理时间10:00,提交的时候乱码 103A00
	var action ="${pageContext.request.contextPath}/admin/preCommonMember/authUpdate2";
	$.post(action, params, function(data){
		  if(data.meta.code=="88"){
       		  alert(data.meta.message);
	       	  window.location.href="${pageContext.request.contextPath}/admin/preCommonMember/authList2";
...
<body style="font-size: 12px; font-family: 微软雅黑">
	...
	<div class="container" style="width: 80%;">
		<div style="float: left;">
			<form method="post" action="authList2" id="form-select" class="form-inline">
 ...
 --%>
							...
							<%-- 	<td><button class="btn" data-backdrop="static"
										...
										onclick="upateUI2('${precommonmember.uid}')">设置</button>
								</td> --%>
								<td><button class="btn" data-backdrop="static"
										data-toggle="modal" data-dismiss="modal"
